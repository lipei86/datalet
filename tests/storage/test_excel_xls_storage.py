...
class ExcelXlsStorageTest(unittest.TestCase):
	...
	# 	s = Excel2003Storage(self.tmpdir + testfile, sheetIndex = 0)
	# 	s.remove(force = True)
	# 	s.create()
	# 	s.remove(force = True)
    #
    ...
    #
	# def test_read(self):
	# 	s = Excel2003Storage(r"tests/test_data/test.xls", sheetIndex = 0)
	# 	dat = s.read()
	...
	# 	s.write(data = dat, overwrite = True)
	# 	newpath = self.tmpdir + os.path.sep + "test_copy2.xls"
	# 	s.copy(path = newpath)
	# 	self.assertTrue(os.path.exists(newpath))
	# 	s.remove()
	# 	Excel2003Storage(newpath, sheetIndex = 0).remove()
    #
	# def test_remove(self):
	# 	with self.assertRaises(StorageNotFoundError):
	# 		s = Excel2003Storage(r"tests/test_data/test_notexists.xls", sheetIndex = 0)
	# 		s.remove()
	# 	s = Excel2003Storage(r"tests/test_data/test_todel.xls", sheetIndex = 0)
	# 	s.create()
	# 	s.remove()
    #
	# def test_clear(self):
